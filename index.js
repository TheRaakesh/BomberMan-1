...
function neighbourhood(cellId) {
    ...
    if (cellId > col && random.includes(cellId - col) && cellId>col ) {
      bombs++;
    }
    ...
    if (cellId % col !== 0 && random.includes(cellId + col - 1) && cellId <(col * row)) {
      bombs++;
    }
    // check the bottom middle cell
    if (cellId < (col * row) && random.includes(cellId + col)) {
      bombs++;
    }
    // check the bottom right corner
    if (cellId % col !== 9 && random.includes(cellId + col + 1) && cellId <(col * row)) {
      bombs++;
    }
    return bombs;
  }
